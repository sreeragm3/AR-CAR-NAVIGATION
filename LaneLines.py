...
class LaneLines:
    ...
    def fit_poly(self, img):
        ...
        # Visualization of Arrow
        arrow_points = np.array(
            [
                np.transpose(np.vstack([center_fitx, ploty])).astype(np.int32)
            ]
        )
        arrow_color = (0, 255, 0)  # Green arrow

        # Draw the shaft of the arrow
        cv2.polylines(out_img, [arrow_points], isClosed=False, color=arrow_color, thickness=60)

        # Add an arrowhead at the end
        arrow_tip = (int(center_fitx[10]), int(ploty[0]))  # Arrow tip at the farthest point away from the camera
        arrow_base_left = (int(center_fitx[20] - 100), int(ploty[20] + 40))  # Base left point closer to the camera
        arrow_base_right = (int(center_fitx[20] + 100), int(ploty[20] + 40))  # Base right point closer to the camera

        # Define the arrowhead shape
        arrow_head = np.array([arrow_tip, arrow_base_left, arrow_base_right])

        # Draw the arrowhead
        cv2.fillPoly(out_img, [arrow_head], arrow_color)
